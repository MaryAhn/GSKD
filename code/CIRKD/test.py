...
class Evaluator(object):
    def __init__(self, args, num_gpus):
        ...
        self.val_dataset = CSTrainValSet(args.data,
    ...
    def eval(self):
        ...
        for i, (image, target, filename) in enumerate(self.val_loader):
            ...
            if self.args.save_pred:
                ...
                seg_pred = pred.cpu().data.numpy()

                colormap = ['#804080', '#F423E8', '#464646', '#66669C', '#BE9999', '#999999', '#FAAA1E', '#DCDC00',
                            '#6B8E23',
                            '#98FB98', '#4682B4', '#DC143C', '#FF0000', '#00008E', '#000046', '#003C64', '#005064',
                            '#0000E6',
                            '#770B20']
                class_name_dict = {0: 'road', 1: 'sidewalk', 2: 'building', 3: 'wall', 4: 'fence', 5: 'pole',
                                   ...
                                   17: 'motorcycle', 18: 'bicycle', -1: 'none'}

                # seg_pred = self.id2trainId(pred, self.id_to_trainid, reverse=True)
                color_map = matplotlib.colors.ListedColormap(colormap)
                # seg_pred = seg_pred.squeeze(0)
                print(os.path.join(args.outdir, os.path.splitext(filename[0])[0] + '.png'))
                plt.imsave(os.path.join(args.outdir, os.path.splitext(filename[0])[0] + '.png'), seg_pred,
                           cmap=color_map, vmin=0, vmax=19)
