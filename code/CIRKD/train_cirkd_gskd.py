...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='ade20k',
                        help='dataset name')
    parser.add_argument('--data', type=str, default='/home/cvlab/datasets/ade20k',
    ...
    parser.add_argument("--gskd-temperature", type=float, default=4.0, help="logits KD temperature")
    ...
    parser.add_argument("--lambda-gskd", type=float, default=5.0, help="lambda descriptor-based")

    parser.add_argument("--N", type=int, default=16, help="N")
...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.distributed:
            ...
            self.criterion_memory_contrast = nn.parallel.DistributedDataParallel(self.criterion_memory_contrast, 
                                                             device_ids=[args.local_rank],
                                                             output_device=args.local_rank)
            self.criterion_kd_desc = nn.parallel.DistributedDataParallel(self.criterion_gskd,
                                                                         device_ids=[args.local_rank],
                                                                         output_device=args.local_rank,
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            gskd_loss = self.args.lambda_gskd * self.criterion_gskd(t_predicts=t_predict, s_feats=s_outputs[-1],
                                                                       t_feats=t_outputs[-1],
                                                                       labels=targets.detach().clone())

            
            losses = task_loss + kd_loss + minibatch_pixel_contrast_loss +\
                memory_pixel_contrast_loss + memory_region_contrast_loss + gskd_loss
            ...
            gskd_losses_reduced = self.reduce_tensor(gskd_loss)
            ...
            if iteration % log_per_iters == 0 and save_to_disk:
                logger.info(
                    ...
                    "|| Mini-batch p2p Loss: {:.4f} || Memory p2p Loss: {:.4f} || Memory p2r Loss: {:.4f} || GSKD Loss: {:.4f} " \
