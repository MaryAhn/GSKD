...
from losses import *
from losses.gskd_loss_B import GSKDB
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='citys',
                        help='dataset name')
    parser.add_argument('--data', type=str, default='/home/cvlab/datasets/cityscapes',
    ...
    parser.add_argument('--pixel-update-freq', type=int, default=16)
    parser.add_argument("--N", type=int, default=16, help="lambda_desc")
    parser.add_argument("--S", type=int, default=4, help="lambda_desc")

    parser.add_argument("--kd-temperature", type=float, default=1.0, help="logits KD temperature")
    parser.add_argument("--gskd-temperature", type=float, default=1.0, help="similarity distribution KD temperature")
    ...
    parser.add_argument("--lambda-gskd", type=float, default=0.5, help="lambda mini-batch-based pixel")
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.criterion_gskd = GSKDB(num_classes=train_dataset.num_class, ignore_label=args.ignore_label,
                                       temperature=args.gskd_temperature,
                                       N=args.N, S=args.S, t_channel=t_channels, s_channel=s_channels).to(self.device)
        ...
        params_list.append(self.criterion_gskd)
        ...
        if args.distributed:
            ...
            self.criterion_memory_contrast = nn.parallel.DistributedDataParallel(self.criterion_gskd,
                                                             device_ids=[args.local_rank],
                                                             output_device=args.local_rank, find_unused_parameters=True)
            self.criterion_gskd = nn.parallel.DistributedDataParallel(self.criterion_gskd,
                                                                      device_ids=[args.local_rank],
                                                                      output_device=args.local_rank,
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            _, t_predict = torch.max(t_outputs[0], dim=1)
            gskd_loss = self.args.lambda_desc * self.criterion_gskd(t_predicts=t_predict, s_feats=s_outputs[-1],
                                                     t_feats=t_outputs[-1].detach(),
                                                     labels=targets.detach().clone())

            D_losses = adv_D_loss
            
            losses = task_loss + kd_loss + cwd_fea_loss + cwd_logit_loss + gskd_loss + adv_G_loss
            ...
            gskd_loss_reduced = self.reduce_mean_tensor(gskd_loss)
            ...
            if iteration % log_per_iters == 0 and save_to_disk:
                logger.info(
                    ...
                    "|| CWD feat Loss: {:.4f} || CWD logit Loss: {:.4f} || Image gskd Loss: {:.4f}" \
