...
class GSKDB(nn.Module):
    def __init__(self, num_classes, ignore_label, temperature, N, S):
        ...
        self.project_head_db = nn.Sequential(
            nn.Conv2d(128, 256, 1, bias=False),
            nn.BatchNorm2d(256),
            nn.ReLU(True),
            nn.Conv2d(256, 256, 1, bias=False)
    ...
    def image_sub_desc(self, student_feat, teacher_feat, temperature):
        ...
        image_desc_cand_tt, image_desc_cand_ts = torch.matmul(teacher_feat, teacher_feat_T), torch.matmul(student_feat,
                                                                                                          student_feat_T)
        image_desc_total_err = 0
        shuffle_map = []
        for k in range(sample):
            shuffled_idxs = torch.randperm(sample).type(torch.cuda.LongTensor)
            shuffle_map.append(shuffled_idxs)
        shuffle_map = torch.stack(shuffle_map)
        for i in range(col):
            desc_tt = image_desc_cand_tt.gather(dim=1, index=shuffle_map[:, i * self.S:(i + 1) * self.S]).reshape(desc)
            desc_ss = image_desc_cand_ts.gather(dim=1, index=shuffle_map[:, i * self.S:(i + 1) * self.S]).reshape(desc)
    ...
    def forward(self, t_predicts, s_feats, t_feats, labels=None):
        ...
        # s_feats = self.project_head_db(s_feats)
