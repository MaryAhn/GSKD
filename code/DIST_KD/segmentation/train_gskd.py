...
def parse_args():
    ...
    parser.add_argument("--gskd-temperature", type=float, default=1.0, help="logits KD temperature")
    
    parser.add_argument("--lambda-kd", type=float, default=1., help="lambda_kd")
    parser.add_argument("--lambda-gskd", type=float, default=0.5, help="lambda_desc")
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.criterion_desc = GSKDB(num_classes=train_dataset.num_class, ignore_label=args.ignore_label, temperature=args.desc_temperature,
                                              N=args.N, S=args.S).to(self.device)
    
        params_list = nn.ModuleList([])
        params_list.append(self.s_model)
        params_list.append(self.criterion_desc)
        ...
        if args.distributed:
            ...
            self.criterion_desc = nn.parallel.DistributedDataParallel(self.criterion_desc,
                                                                device_ids=[args.local_rank],
                                                                output_device=args.local_rank)
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            desc_loss = self.args.lambda_desc * self.criterion_desc(t_predicts=t_predict, s_feats=s_outputs[-1], t_feats=t_outputs[-1], labels=targets.detach().clone())
            ...
            losses = task_loss + kd_loss + desc_loss
            ...
            desc_losses_reduced = self.reduce_mean_tensor(desc_loss)
